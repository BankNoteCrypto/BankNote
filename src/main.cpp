...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 2);
static CBigNum bnProofOfStakeLimit(~uint256(0) >> 4);

static CBigNum bnProofOfWorkLimitTestNet(~uint256(0) >> 2);
static CBigNum bnProofOfStakeLimitTestNet(~uint256(0) >> 4);
...
int64 nChainStartTime = 1400483427;
...
{
    int64 nSubsidy = 5555555 * COIN;

    if(nHeight < 6)
    {
	nSubsidy = 1000000000ULL * COIN;
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BankNote
    bool fStrictPayToScriptHash = true; // Always active in BankNote
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BankNote", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "18 May 2014 Tosh W wins DOJECAR race.";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("abf961981a5c6e76c0e6ce569ea6b2731d6f4945e0328360c1ebafb992e6537a"));
